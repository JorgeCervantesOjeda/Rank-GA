...
sea distinto del valor actual es \(1 - 1/\texttt{numValues}\). El caso binario
es el caso particular \(\texttt{numValues}=2\), por lo que
\[
G = 1 - \frac{1}{2} = 0.5.
\]
En una cadena binaria, mutar cada bit con intensidad \(G=0.5\) hace que cada
cadena binaria tenga la misma probabilidad de aparecer. Para una cadena de
longitud \(n\), cualquier cadena espec\'ifica tiene probabilidad
\[
...
mutaci\'on resulta positivo y la mutaci\'on crece con el rango.
