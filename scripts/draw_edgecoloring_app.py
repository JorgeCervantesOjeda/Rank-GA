...
class GraphRendererApp:
    def __init__(self, root):
        ...
        with open('edge_coloring_20_39.txt', 'r') as file:
            ...
            self.vertices = list(range(int(file.readline().strip())))
            # Read color codes as integers
            self.edge_coloring = [int(code)
        ...
        for i, (code, initial_value) in enumerate(zip(self.unique_color_codes, initial_checked_values)):
            var = IntVar(value=initial_value)
            checkbox = Checkbutton(
                self.frame, text=f"Color {code}", variable=var, command=self.update_graph)
    ...
    def update_graph(self):
        ...
        selected_color_codes = [code for code,
                                var in self.color_checkboxes if var.get() == 1]
        ...
        nx.draw(custom_graph, positions, ax=self.ax, with_labels=True, node_color='lightgray', font_weight='bold',
                node_size=1000, font_size=12,
                edge_color=edge_colors, style=edge_styles, width=edge_widths,
                cmap=plt.cm.rainbow, font_color='black', arrows=False)  # Disable arrows for an undirected graph
        ...
        for i, code in enumerate(unique_color_codes):
            ...
            legend_handles.append(
                plt.Line2D([0], [0], color=color, linestyle=style,
                           linewidth=width, label=f'Color {code}')
            )
