...
//  public static int[][] trajsAdj;
  ...
  public static ArrayList<TrajTriplet>[][] trajTriplets;
  ...
  public ProblemHeawoodRainbow( int num_colors ) {
    ...
    //**************************************************
    trajTriplets = new ArrayList[ 14 ][ 14 ];
    for( int i = 0;
         i < 13;
         i++ ) // i: origen
    {
      for( int j = i + 1;
           ...
           j++ )// j: destino
      {
        trajTriplets[ i ][ j ] = findDisjointTrajTriplets(
        trajectories[ i ][ j ] );
      }
    }
    writeTrajsDisj();
        ...
        for( int j = i + 1;
          ...
          for( int m = 0;
            ...
            for( int n = 0;
              ...
              wr.append( n + "\t" );
              for( int k = 0;
                   k < trajectories[ i ][ j ][ ( trajTriplets[ i ][ j ]
                                                .get( m ) )
                     .getTrajId( n ) ].length;
                   k++ ) {
                wr.append(
                  trajectories[ i ][ j ][ ( trajTriplets[ i ][ j ]
                                           .get( m ) )
                    .getTrajId( n ) ][ k ]
  ...
  public static ArrayList<TrajTriplet> findDisjointTrajTriplets(
    ...
    for( int i = 0;
         i < trayectorias.length - 2;
         i++ ) {
      for( int j = i + 1;
           j < trayectorias.length - 1;
           j++ ) {
        if( disjunta( i,
                      j,
                      trayectorias ) ) {
          for( int k = j + 1;
               k < trayectorias.length;
               k++ ) {
            if( disjunta( i,
                          k,
                          trayectorias ) && disjunta( j,
                                                      k,
                                                      trayectorias ) ) {
  ...
  public static boolean disjunta( int i,
    ...
    for( int k = 1;
         k < trayectorias[ i ].length - 1;
         k++ ) {
      nodoUsado[ trayectorias[ i ][ k ] ] = true;
    }
    for( int k = 1;
         k < trayectorias[ j ].length - 1;
         k++ ) {
  ...
  private double numColoresDist( int[] trayectoria,
    ...
    for( int i = 0;
         i < NUM_COLORS;
         i++ ) { // ningun color se ha usado
      coloresUsados[ i ] = false;
    }

    int color;
    for( int nodo = 1;
         nodo < trayectoria.length;
         nodo++ ) {
      color = genome[ getIndex( trayectoria[ nodo - 1 ],
                                trayectoria[ nodo ] ) ].getIntValue();
